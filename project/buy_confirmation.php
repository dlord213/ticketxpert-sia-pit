...
try {
  ...
  if (isset($_GET['transaction_id']) && is_numeric($_GET['transaction_id'])) {
    ...
    $total_payment = $transaction_details['price']  * $transaction_details['amount'];
    $formattedTotal = number_format($total_payment, 2, '.', ',');
  }
...
<body class="bg-slate-100">
  <main class="min-h-[100vh] flex flex-col justify-center items-center">
    <div class="flex flex-row gap-4 items-center rounded-lg bg-white my-4 text-slate-700 p-4 shadow-md">
    ...
    <div class="flex flex-row gap-4 justify-center h-full">
      <div class="flex flex-col drop-shadow-xl bg-white rounded-lg w-[240px]" id="content-container">
        <img src=<?php echo $transaction_details['portrait_image_url'] ?> class="w-full object-fit rounded-tr-lg rounded-tl-lg rounded-lg" />
        <div class="p-4">
          ...
          <p class="text-slate-600"><b>Amount of tickets:</b> <?php echo $transaction_details['amount'] ?></p>
          <p class="text-slate-600"><b>Total:</b> ₱ <?php echo $formattedTotal ?></p>
          <p class="text-slate-600"><b>Location:</b> <?php echo $transaction_details['location'] ?></p>
        </div>
      </div>
      <form class="flex flex-col p-4 bg-white rounded-lg drop-shadow-xl gap-1" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
        ...
        <p class="text-slate-600 w-[240px]">Please scan the QR code in your GCash app and pay, enter the reference number of the transaction after paying.</p>
