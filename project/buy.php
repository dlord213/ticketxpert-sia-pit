...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  try {
    ...
    $preparedStmt = $connection->prepare('INSERT INTO transaction(attendee_id, ticket_id, transaction_date, amount, status) VALUES (?, ?, CURRENT_TIMESTAMP, ?, ?)');
    $attendeeExist = $connection->query("SELECT * FROM attendee WHERE user_id = " . $_SESSION['user_id'])->fetch(PDO::FETCH_ASSOC);

    if (!$attendeeExist) {
      $preparedInsertAttendeeStmt = $connection->prepare("INSERT INTO attendee (user_id) VALUES (?)");
      $preparedInsertAttendeeStmt->execute([$_SESSION['user_id']]);

      $attendeeExist = $connection->query("SELECT * FROM attendee WHERE user_id = " . $_SESSION['user_id'])->fetch(PDO::FETCH_ASSOC);
    }

    if ($attendeeExist) {
      $preparedStmt->execute([$attendeeExist['user_id'], $ticket_id, $_POST['ticket_quantity'], 'BOUGHT']);
